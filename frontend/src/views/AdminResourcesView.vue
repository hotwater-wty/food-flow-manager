...
import {
  ...
  getAdminCategories, getAdminDishes, getAdminReservationDetail, getAdminReservations, getAdminTables, getEmployees,
...
<template>
  <section class="reservation-view"><div class="reservation-heading"><p class="eyebrow">管理端资料</p><h1>资料维护工作台</h1><p>按资源查看和维护当前后端已提供的管理数据。</p></div>
    ...
    <div v-if="tab === 'employees' && employees.length" class="resource-list"><article v-for="item in employees" :key="item.employeeId" class="reservation-card"><strong>{{ item.name }}</strong><span>{{ item.phone }} · {{ item.role === 2 ? '店长' : '店员' }} · {{ employeeText(item.status) }}</span><button class="primary-outline-button" type="button" @click="run(() => setEmployeeEnabled(item.employeeId, item.status !== 1), '员工状态已更新')">{{ item.status === 1 ? '禁用' : '启用' }}</button></article></div>
