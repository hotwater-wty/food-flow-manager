...
后端 V1/V2 核心业务和工程增强已完成并冻结扩张；根目录 `frontend/` 已完成 Vue 3 + TypeScript + Vite 工程初始化，并已接入顾客认证、预约/开台、菜单、购物车、订单创建/列表/详情，以及员工认证、管理端订单工作台和会话工作台。管理端使用独立员工 JWT，订单支持分页、详情和状态推进，会话支持分页、取消等待和清台。真实有数据成功分支仍需在存在业务数据时复验；第二批资料维护页面尚未实现。
