...
后端 V1/V2 核心业务和工程增强已完成并冻结扩张；根目录 `frontend/` 已完成 Vue 3 + TypeScript + Vite 工程初始化，并已接入顾客认证、请求鉴权、受保护路由、空闲桌位查询、预约创建提交、预约列表、详情和取消链路。当前预约成功、有数据列表和取消成功分支仍需在存在 `FREE` 桌位及预约记录时复验，模拟扫码开台、点餐和商户端尚未实现。前端文档以真实后端接口为边界，先完成顾客端和商户端各一条纵向业务链路，再决定是否扩展管理资料维护等第二批功能。
