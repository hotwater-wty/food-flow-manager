...
后端 V1/V2 核心业务和工程增强已完成并冻结扩张；根目录已预留 `frontend/`，但尚未初始化 Vue 工程和前端源码。下一阶段是建立前端基础工程并实现最小可演示闭环。前端文档以真实后端接口为边界，先完成顾客端和商户端各一条纵向业务链路，再决定是否扩展管理资料维护等第二批功能。
...
- `frontend/` 完成工程初始化，并通过类型检查、构建和最小登录验收。
