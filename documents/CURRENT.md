...
后端 V1/V2 核心业务和工程增强已完成并冻结扩张；根目录 `frontend/` 已完成 Vue 3 + TypeScript + Vite 工程初始化，并已接入顾客认证、请求鉴权、受保护路由、空闲桌位查询、预约创建提交、预约列表/详情/取消、非预约模拟扫码开台、预约到店开台、菜单浏览、分类筛选、内存购物车、订单创建、订单列表和详情。当前预约/开台成功分支、菜单有数据分支及订单成功分支仍需在存在真实业务数据时复验；商户端尚未实现。前端文档以真实后端接口为边界，先完成顾客端和商户端各一条纵向业务链路，再决定是否扩展管理资料维护等第二批功能。
