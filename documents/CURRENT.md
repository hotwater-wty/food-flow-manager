...
后端 V1/V2 核心业务和工程增强已完成并冻结扩张；根目录 `frontend/` 已完成 Vue 3 + TypeScript + Vite 工程初始化，但尚未实现业务切片。下一阶段是接入真实认证接口并实现最小可演示闭环。前端文档以真实后端接口为边界，先完成顾客端和商户端各一条纵向业务链路，再决定是否扩展管理资料维护等第二批功能。
...
| 前端产品与技术决策 | [`frontend/01-前端项目决策总览.md`](frontend/01-前端项目决策总览.md) | 目标规格，尚未实现 |
...
- 前端完成一个可运行的纵向切片并形成浏览器验收证据。
- `frontend/` 完成真实认证切片，并通过类型检查、构建和浏览器登录验收。
