...
> 文档状态：历史记录，已按当前缓存抽象校准关键结论
> 当前事实入口：[后端技术选型与当前依赖](../../architecture/backend/技术选型与依赖规划.md)
> 历史边界：本文保留序列化问题和初版方案；缓存键、TTL、失效逻辑和封装边界以当前源码为准。

## 1. 解决什么问题

本文记录项目引入 Redis 缓存时形成的一套基础方案。文中的 V2 是当时阶段名称，不是当前开发分类。
...
当前仓库已经将 JSON 读写抽取为 `common.utils.CacheUtil`，并在 `DishCacheClient`、`DishCategoryCacheClient`、`AccountStatusCacheClient` 中提供业务级缓存边界。菜品/分类缓存采用旁路缓存和失效删除；账号状态缓存用于登录态相关状态核验。

下列流程是菜品列表缓存形成时的旁路缓存模式：
...
说明：`keys` 在数据量很大时不适合在线上高频使用。当前实现仍保留这一受限策略；维护 key 集合或按确定条件删除指定 key 是候选优化，并未实现。
...
该写法能跑通，但多个模块使用后会产生重复代码。

这一演进已完成：当前 `CacheUtil` 统一 JSON 序列化、空值缓存和 TTL 处理，各业务缓存 Client 负责键名、查询回源和失效范围。下面的 `CacheClient` 是当时的示意代码，不是当前类名或完整 API：
...
当前项目已不再是“业务代码直写、等待抽取”的状态。新增缓存场景应优先复用 `CacheUtil` 与已有业务 Client；是否增加新的 Client 取决于是否存在明确的键空间与失效责任。
...
6. 写缓存时设置 TTL。
7. 数据变更后删除相关缓存。
8. 多处重复时抽取通用工具，并以业务 CacheClient 明确键与失效责任。
