...
> 文档状态：历史记录，已按当前分页实现校准关键结论
> 当前事实入口：[后端接口规范](../../architecture/backend/接口规范设计.md)
> 历史边界：本文保留分页改造的样板与推理；接口范围以当前 Controller、DTO 和 Service 为准。

本文档记录在当前 Spring Boot + MyBatis-Plus 项目中实现分页查询的基本方式。重点说明 `MyBatisConfig`、`PageResult`、`Page<T>` 的职责，以及如何把一个普通列表接口改造成分页接口。

## 1. 解决什么问题

在分页能力引入前，多个管理端列表接口直接返回全量数据。
...
该改造已经完成：管理端订单、预约、开台会话、菜品、分类、桌位和员工列表均返回 `PageResult`；用户端列表仍按当前接口契约返回普通列表，不应被前端假设为分页接口。
...
当前仓库已抽取 `common.dto.PageQueryDTO`，多数管理端列表使用该对象；会话与订单保留各自的业务筛选 DTO。上面的代码仅说明当时的改造思路，不是新增接口的唯一模板。
...
用户端的订单、预约、菜品、分类和桌位查询并非全部分页。前端接入必须按 [后端接口规范](../../architecture/backend/接口规范设计.md) 的单个路径确认参数与响应结构。
