...
| 当前发展计划 | [`planning/project-development-plan.md`](planning/project-development-plan.md) |
| 前端产品、页面和接口事实 | [`frontend/`](frontend/) |
...
仓库代码边界：`backend/` 是 Spring Boot Maven 工程；`apps/customer` 与 `apps/admin` 是两个独立 Vite 应用，`packages/shared` 保存共享契约；根目录 `docker-compose.yml` 当前编排 MySQL、Redis 和后端。

## 单一事实源

- 可运行行为、接口实际语义和完成范围：以 `backend/` 当前代码、测试、`README.md` 和 `assets/schema.sql` 为准。
- 当前任务与优先级：以 [`planning/project-development-plan.md`](planning/project-development-plan.md) 为准。
...
1. 当前仍会影响开发决策的产品、接口或工程事实，放入 `frontend/` 或 `architecture/backend/`。
2. 可复用的流程、模板和维护约定，放入 `guides/`。
3. 已发生的复盘、取舍、排错和专题沉淀，放入 `records/`。
4. 当前计划只放入 `planning/`；版本结束、被新结论替代或仅保留参考的内容，按主题移入 `archive/`。
