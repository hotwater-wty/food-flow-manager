...
async function submit() {
  ...
  try {
    ...
    await router.push(typeof route.query.redirect === 'string' ? route.query.redirect : '/orders')
  } catch (error) {
...
<template>
  <section class="auth-view">
    ...
    <form class="auth-form" @submit.prevent="submit">
      <label>手机号<input v-model="phone" required autocomplete="username" /></label
      ><label>密码<input v-model="password" required type="password" autocomplete="current-password" /></label>
      <p v-if="errorMessage" class="feedback feedback-error" role="alert">{{ errorMessage }}</p>
      <button type="submit" :disabled="isSubmitting">{{ isSubmitting ? '登录中...' : '登录管理端' }}</button>
    </form>
  </section>
</template>
