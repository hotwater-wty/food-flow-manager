...
> 当前边界：后端业务、缓存和部署已形成可运行基线；根目录 `frontend/` 已完成 Vue 3 + TypeScript + Vite 前端首版，覆盖顾客注册/登录、预约/开台、菜单详情、内存购物车、订单，以及员工认证、管理订单/会话和第二批资料维护工作台。前端本阶段已完成并停止扩展；支付、持久化购物车、真实二维码等候选能力不属于当前完成范围。
...
前端目标规格采用 Vue 3、TypeScript、Vite、Pinia、Vue Router 和 Axios；当前 `frontend/` 已完成首版闭环，未引入 Vant 或 Element Plus，页面使用项目内样式和原生表单控件。
...
前端首版已按“顾客登录 -> 预约或模拟扫码 -> 点餐 -> 查看订单；店员登录 -> 处理订单 -> 清台”的真实后端闭环完成验收；后续新增能力须另立计划。
