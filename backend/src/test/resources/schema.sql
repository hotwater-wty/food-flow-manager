...
CREATE TABLE IF NOT EXISTS `dining_session` (
  ...
  `active_flag` TINYINT DEFAULT NULL COMMENT '活跃标识：1活跃，NULL非活跃',
  ...
  UNIQUE KEY `uk_dining_session_user_active` (`user_id`, `active_flag`),
  UNIQUE KEY `uk_dining_session_table_active` (`table_id`, `active_flag`),
